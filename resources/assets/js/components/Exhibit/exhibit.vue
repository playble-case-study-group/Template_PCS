<template>
    <div id="exhibit" class="container main">
        <div class="heading">
            <h3 v-if="exhibition.group_id != 0">{{ exhibition.group_id }}'s Exhibit</h3>
            <h3 v-else>{{ this.$store.state.user.name }}'s Exhibit</h3>
        </div>
        <div class="editor col-sm-11">
            <vue-mce
                    v-model="submission"
                    :config="config"
                    ref="ref"
            ...
            <button @click="saveChanges" class="btn btn-success save">Save</button>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        data: function() {
            return {
                ...
                config: {
                    ...
                    toolbar1: 'preview | formatselect fontsizeselect | bold italic strikethrough forecolor backcolor | link | alignleft aligncenter alignright alignjustify  | numlist bullist outdent indent',
                    image_list: [],
        ...
        methods: {
            handleInit (editor) {
                ...
                if(this.exhibition){
                    this.submission = this.exhibition.exhibition;
                }
            },
